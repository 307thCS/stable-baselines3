...
class Actor(BasePolicy):
    """
    Actor network (policy) for TD3-M.
    ...
    :param num_ideas: Number of candidate actions produced by actor
    """
    ...
    def reshape_output(self, output):
        output_shape = th.tensor(output.shape).tolist()
        output_shape.append(output_shape[-1] // self.num_ideas)
        output_shape[-2] = self.num_ideas
...
class Critic(BaseModel):
    """
    Critic networks for TD3-M.
    It represents the action-state value function (Q-value function).
    Compared to A2C/PPO critics, this one represents the Q-value
    and takes the continuous action as input. It is concatenated with the state
    and then fed to the network which outputs a single value: Q(s, a).
    Like TD3, it creates two critic networks used to reduce overestimation
    ...
    def q1_forward(self, obs: th.Tensor, actions: th.Tensor, idx: int = 0) -> th.Tensor:
        """
        Only predict the Q-value using one network.
        This allows you to reduce computation when all the estimates are not needed
...
class TD3MPolicy(BasePolicy):
    ...
    :param share_features_extractor: Whether to share or not the features extractor
        between the actor and the critic (this saves computation time)
    :param num_ideas: Number of candidate actions generated by actor
    """
...
MlpPolicy = TD3MPolicy

class CnnPolicy(TD3MPolicy):
    """
    Policy class (with both actor and critic) for TD3.
...
class MultiInputPolicy(TD3MPolicy):
    """
